import {
  countryLabel, defaultLang, itemLabel, itemRoute, mdInline, mdStrip, partnerById, projectName, tr, translations,
...
// is declared here is only what is this exhibition's: the field
// specification (legacy DatabaseItem.vue's `objectData`, field for field),
// the photos in their order, the related tile, and the citation whose
// project name follows this exhibition's own rule. Every label is an entry
...
// a language this exhibition does not offer) they fall back to English,
// which is where legacy pinned them anyway.
//
// Legacy shows both descriptions when both exist, with the short one
// collapsed behind a toggle (the view's, folded after the description); when
// only one exists it is relabelled plain "Description". EPM records are the
...
const fields = [
  ...
  // The only description there is, under the plain label; the toggled one is
  // the view's, folded after `description` when both exist.
...
export const itemSheet = {
  ...
  // sheet, so the platform reads them off another row when the active one
  // has neither — the one place a page reads across languages, and only for
  // proper names.
  ...
  citation: {
    ...
    project: (record) => projectName(record),
  },
  related: {
    variant: 'grid',
